...
const AvailabilitySchedule = () => {
    ...
    const removeSlot = (e: React.MouseEvent<HTMLButtonElement>, dayIndex: number, slotIndex: number) => {
        e.preventDefault();
            ...
            <form onSubmit={handleSubmit}>
                <div className="max-h-[600px] overflow-y-scroll">
                    {schedule.map((item, index) => (
                        <div key={item.day} className="flex gap-x-12">
                            ...
                            <div className="flex-7 flex flex-col justify-center">
                                {item.enabled ? (
                                    item.slots.map((slot, slotIndex) => (
                                        <div key={slotIndex} className="flex justify-between p-2 gap-10">
                                            <div className="flex-2 flex items-center">
                                                <label htmlFor="" className="text-xl mr-4">from</label>
                                                <input
                                                    type="time"
                                                    className="flex-3 border-2 rounded-xl py-4 px-4"
                                                    onChange={(e) => updateSlot(index, slotIndex, 'from', e.target.value)}
                                                />
                                            </div>
                                            <div className="flex-2 flex  items-center">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => updateSlot(index, slotIndex, 'to', e.target.value)}
                                                />
                                            </div>
                                            <div className="flex-1 flex justify-around items-center">
                                                <Button className="bg-blue-600" onClick={(e) => removeSlot(e, index, slotIndex)}><Minus /></Button>
                                                <Button className="bg-blue-600" onClick={(e) => addSlot(e, index)}><Plus /></Button>
                                            </div>
                                        </div>
                                    ))
                                ) : (
                                    <div className="flex justify-between p-2 text-xl"><span>Unavailable</span></div>
                                )}
                            </div>
                        </div>
                    ))}
